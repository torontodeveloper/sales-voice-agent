# ABC Energy Sales Voice Agent — Technical Whitepaper                                                              
  ...
  4. Llama uses RMSNorm(Root Mean Square Normalization) which is computationally simplers and more efficient than LayerNorm which uses Mean and standard deviation for activations
  5. Interestly Llama uses SiLU(Sigmoid Linear Unit)  which is faster  compared to ReLU(Rectified Linear Unit). Also Silu keeps smaller negative values with smooth gradient flow                                                        
  ...
  I am using Weights& Biases and Langsmith instead of Langfuse since I am opensource contributor to Langchain as both Langsmith and Langfuse does the same task of observability and monitoring for LLMs
                  
  MLflow (Training) - I have used MLflow in all my projects but Weights&Biases is equally popular in enterprise
                                                                                                                     
  Tracks SFT/DPO experiments — loss curves, hyperparameters, model versions.     
                    
  <img src="./mocks/hyperparameters.png">
